        ...
        if row["prod_latest"] and row["stage_latest"]:
            try:
                ...
                if t1 and t2:
                    ...
                    row["diff_hours"] = round(abs((t2 - t1).total_seconds()) / 3600, 2)
                    row["pass"] = row["diff_hours"] <= MAX_TIME_DIFF_HOURS
                    if not row["pass"]:
                        results["passed"] = False
            except Exception:
                row["diff_hours"] = None
...
"""
    ...
    html += f"""
<div class="footer">Generated by FH Verification script (mode={run_mode}). Thresholds: variance &lt;= {VARIANCE_THRESHOLD_PERCENT}%, time diff &lt;= {MAX_TIME_DIFF_HOURS}h, write-count variance &lt;= {getattr(sys.modules[__name__], 'WRITE_COUNT_VARIANCE_THRESHOLD_PERCENT', 10)}%.</div>
</body>
</html>
"""
    with open(output_path, "w", encoding="utf-8") as f:
